...
Last reviewed: September 3, 2026. These are priorities, not promised release dates. Every implemented milestone below, including the guided journey beyond the first month and the second-month authored practice, is deployed to SystemDesigner.net (release `041b246b`); live guest checks passed. The remaining product and validation work is described below.
...
Twenty-three lessons across the second and third months now have hand-authored practice: the ten that open month two (replication and sharding, consistent hashing, SSTables, Cassandra, DynamoDB, Memcached, Elasticsearch, Lucene, object storage, CloudFront), the six service-interface lessons (API design, API paradigms, GraphQL, gRPC, WebSocket, HTTP/2 and HTTP/3), and the seven asynchronous-systems lessons (communication patterns, message queues, Kafka, RabbitMQ, SQS, event-driven architecture, Temporal). They add 112 groups and 336 variants of capacity, quorum, budget, and fan-out calculations, ordered protocols, and scenario decisions whose answers were checked against the lesson sources. Authored practice now totals 46 packs, 181 groups, and 543 variants. The 156 remaining derived packs name the lesson section each matching task comes from, give hints that recall the lesson’s key idea instead of listing the answer, and explain each part’s responsibility after checking. Every lesson session links its registry prerequisites from “Go a little deeper”.
...
Curriculum improvement continues with real learner evidence: check placement difficulty, evaluate retention and transfer, refine broad lesson-level skills, and keep replacing derived matching and recognition tasks with authored calculations, diagnoses, and trade-offs. The resilient-services unit that closes month three, then production, security, and the design lab in month four, are the next authoring targets. The current review scheduler remains an explainable heuristic rather than a calibrated proficiency estimate. Matching a source model is useful practice, but does not establish mastery of its whole subject.
