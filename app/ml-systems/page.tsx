...
import { generateMLSystemsLearningPaths } from '@/lib/nav-generators';

export default function MLSystemsPage() {
  const { progress, loading, error, isCompleted, getCompletionPercentage, getCompletedCount } = useProgressTracking('ml-systems');
  
  // Generate learning paths from content registry (single source of truth)
  const learningPaths = generateMLSystemsLearningPaths();
        ...
        {!loading && progressPercentage === 100 && (
          <div className="mb-6 rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/20 p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h3 className="text-xl font-bold text-emerald-900 dark:text-emerald-100">🎉 ML Systems Complete!</h3>
            </div>
            <p className="text-emerald-700 dark:text-emerald-300 mb-4">
              Congratulations! You've mastered traditional ML systems. Ready to explore modern AI with LLMs and GenAI?
            </p>
            ...
            <div className="grid gap-3">
              {path.lessons.map((lesson, lessonIndex) => {
                ...
                return (
                  <Link
                    key={lesson.slug}
                    href={`/ml-systems/${lesson.slug}` as any}
