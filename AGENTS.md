...
- `/learn` has four courses, 46 units, 273 sessions, and 236 explicit skills. Placement and adaptive review span every course; a 326-study-day guided journey in 15 parts connects every one of the 273 sessions across all four courses in course order, with 53 review days (106 review tasks), 25 milestones, and a coding project closing each of the first four months. All 202 lesson sessions use hand-authored practice: 202 packs with 961 groups and 2,883 variants (990 ordering, 1,068 decision, 540 calculation, 285 matching exercises). No lesson uses generated practice any more. Use the shared exercise schema; authored packs override generated practice through the course outline. A coding exercise normally exports one function; declaring `exports` and giving a test `calls` instead of `args` drives an ordered sequence across several functions, rebuilt per test so state does not leak between cases. `pnpm validate:practice`, part of `pnpm validate:content`, enforces the authoring contract: a choice answer must not be findable by length, ordering items are stored shuffled rather than in solved order, every calculation names at least two plausible wrong values, and units stay to one or two words. `pnpm qa:practice-variants` walks the second and third variant of every group in a browser; the curriculum sweep only ever reaches the first. Version 5 progress restores interrupted attempts and retains bounded coding draft history. Keep learning and progression primary; account sync and backups belong in settings.
- Author course organization in `content/learning/course-outline.json` and journey parts in the files listed under `journeySources`; preserve existing step and day IDs, and run `pnpm generate:learning` after changing course sources, journey parts, or assessments. `pnpm validate:learning` checks generated catalog/session/checkpoint drift.
