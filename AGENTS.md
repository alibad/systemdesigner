...
- `/learn` has four courses, 43 units, 264 sessions, and 227 explicit skills. Placement and adaptive review span every course; a 130-study-day guided journey in five parts connects every design and coding session with 28 review days, 16 milestones, and a coding project closing each month. All 202 lesson sessions use hand-authored practice: 202 packs with 961 groups and 2,883 variants (990 ordering, 1,068 decision, 540 calculation, 285 matching exercises). No lesson uses generated practice any more. Use the shared exercise schema; authored packs override generated practice through the course outline. `pnpm validate:practice`, part of `pnpm validate:content`, enforces the authoring contract: a choice answer must not be findable by length, ordering items are stored shuffled rather than in solved order, every calculation names at least two plausible wrong values, and units stay to one or two words. Version 5 progress restores interrupted attempts and retains bounded coding draft history. Keep learning and progression primary; account sync and backups belong in settings.
