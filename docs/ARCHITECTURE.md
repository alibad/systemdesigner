...
If you're here to *write or fix a lesson*, you may want [docs/CONTENT_GUIDE.md](./CONTENT_GUIDE.md) instead. To *set up your machine*, see [docs/DEVELOPMENT.md](./DEVELOPMENT.md). The authoritative content rules live in [AGENTS.md](../AGENTS.md).
...
├── AGENTS.md                    # Authoritative content and agent standards
...
**Diagramming Sandbox (tldraw).** A whiteboard for sketching architectures, with saved diagrams persisted to Firebase for signed-in users. Never call `store.loadSnapshot()` on a `TLStore`; snapshots without compatible schema metadata can fail during migration. Convert persisted records to an object keyed by record ID and pass them through the component's `snapshot` prop:

```tsx
const recordsMap = Object.fromEntries(
  pageData.records.map(record => [record.id, record])
);
...
Keep export or preview instances isolated from the interactive store. The `snapshot` prop owns initialization, while subsequent edits should use supported store operations such as `store.put(...)`.
...
| Touch the diagram sandbox | Read the tldraw snapshot and persistence rule in **Diagramming Sandbox** above, then work in `app/whiteboard/` and the related project components |
...
- [AGENTS.md](../AGENTS.md) — authoritative content-authoring and agent conventions.
