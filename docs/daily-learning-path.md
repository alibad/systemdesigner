...
`/learn` is the daily practice entry point. The homepage and main navigation link to it. The course expansion includes four courses, 43 units, and 264 sessions: 202 sessions adapted from existing lessons, 37 mixed quiz checkpoints, and 25 JavaScript exercises. The existing registry-backed lessons remain the source for deeper reading.
...
The guided journey has five parts and 130 study days. `content/learning/first-month.json` defines the opening 30 days (26 distinct sessions, eight review tasks, four milestones, and the `code-link-service` capstone). `second-month.json`, `third-month.json`, `fourth-month.json`, and `fifth-part.json` continue the numbering through days 31–130: every remaining design and coding session appears exactly once in course order, with 28 review days (56 review tasks), 16 milestones, and a coding project closing each 30-day month. The outline lists the sources under `journeySources`; the generator validates part IDs, continuous day numbers, known sessions, course-order prerequisites, review-after-practice, project days that build code, a milestone at the end of every part, and writes the combined `content/learning/journey.json` that `lib/learning-journey.ts` loads. Day IDs are stable (`day-01` … `day-130`), so existing journey tasks keep their keys.
...
The original fourteen packs cover the first three design units. Nine packs extend authored practice into concurrency, Redis, GenAI, and ML. Ten packs written on September 3 cover the data-at-scale and fast-reads units that open the second month, and thirteen more written on September 4 cover the service-interfaces unit (API design, API paradigms, GraphQL, gRPC, WebSocket, HTTP/2 and HTTP/3) and the asynchronous-systems unit (communication patterns, message queues, Kafka, RabbitMQ, SQS, event-driven architecture, Temporal): 46 packs, 181 groups, and 543 variants in total. Each authored session presents three to five exercises drawn from calculation, ordering, and scenario-decision tasks whose answer keys were checked against the lesson sources. The remaining 156 lesson sessions use packs derived from their existing source lessons, bringing the whole curriculum to 805 groups and 2,091 variants. `lib/skill-exercise-schema.ts` validates numeric answers, complete ordering permutations, matching pairs, and choice answers with option-specific feedback. `SkillPractice.tsx` renders numbers, keyboard-operable ordering, and role matching; scenario choices use the existing shared `InteractiveQuiz`. Concept context precedes every task, hints remain optional, and normal practice allows unlimited retries. Reviews rotate the chosen variant. Derived practice is explicitly marked with its source; it is not counted as newly hand-authored content.
...
Every noncoding lesson now has a shared exercise pack. Forty-six authored packs contain 543 variants; 156 derived packs reuse the source lesson’s actual responsibilities and questions. Derived matching asks learners to connect a part with its job; it never treats optional or parallel process steps as a mandatory sequence. Derived prompts name the lesson section they come from, hints recall the lesson’s takeaway or key idea instead of listing the answer, and the explanation restates each part’s responsibility after checking. Short descriptions remain linked to the complete source explanation. Source questions and explanations are retained in review rotation. Larger source models load individually on request; the guided lesson remains the main flow.
