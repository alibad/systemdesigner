...
Every one of the 65 system-design lessons now has a hand-authored pack: the original fourteen for the first three units, ten written on September 3 for data-at-scale and fast-reads, and thirty-nine written on September 4 for service interfaces, asynchronous systems, resilient services, production, security, the design lab, and the four case studies. Seven further packs cover concurrency, Redis, and selected GenAI and ML lessons: 72 packs, 311 groups, and 933 variants in total. Each authored session presents three to five exercises drawn from calculation, ordering, and scenario-decision tasks whose answer keys were checked against the lesson sources. The remaining 130 lesson sessions, all in the GenAI and ML courses, use packs derived from their existing source lessons, bringing the whole curriculum to 831 groups and 2,208 variants. `lib/skill-exercise-schema.ts` validates numeric answers, complete ordering permutations, matching pairs, and choice answers with option-specific feedback. `SkillPractice.tsx` renders numbers, keyboard-operable ordering, and role matching; scenario choices use the existing shared `InteractiveQuiz`. Concept context precedes every task, hints remain optional, and normal practice allows unlimited retries. Reviews rotate the chosen variant. Derived practice is explicitly marked with its source; it is not counted as newly hand-authored content.
