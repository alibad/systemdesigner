...
## Guided first month and mobile app

`content/learning/first-month.json` defines 30 study days spanning 26 distinct sessions and eight review tasks. The generator validates day IDs/order, known sessions, prerequisites, authored practice coverage, and the final project. This sequence combines the first three design units with the first two coding units. The new `code-link-service` capstone is co-located with the URL-shortener lesson and finishes the collections unit.

`lib/learning-journey.ts` derives the first incomplete day. Ordinary days accept completed practice or valid unit placement; review days need a recorded full session at the current revision. `FirstMonth.tsx` renders onboarding, the current day, the complete plan, and four milestones. Milestones require every earlier day, including reviews. There are no calendar locks: a study day describes curriculum progress rather than elapsed time.
...
- Skills and revisions: the catalog has 227 explicit skills. The generator hashes lesson, assessment, and exercise sources into skill revisions, then hashes each unit's steps into its revision. Evidence and placement grants must match current content. Existing lesson completions remain intact after content changes.
...
The first-month browser suite completes every day through the visible UI, checks mistakes and hints, reloads a partially completed review day, rejects an incorrect cache-invalidation project, restores its draft, and verifies all four milestones. Against a production server, `LEARNING_QA_OFFLINE=1 pnpm qa:first-month` additionally checks offline reload and exercise execution. Real-device home-screen installation and retention studies are separate checks.
...
Every noncoding lesson now has a shared exercise pack. Twenty-three authored packs contain 207 variants; 179 derived packs reuse the source lesson’s actual responsibilities and questions. Derived matching asks learners to connect a part with its job; it never treats optional or parallel process steps as a mandatory sequence. Short descriptions remain linked to the complete source explanation. Source questions and explanations are retained in review rotation. Larger source models load individually on request; the guided lesson remains the main flow.
