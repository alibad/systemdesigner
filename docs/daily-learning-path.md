...
The original fourteen packs cover the first three design units. Nine packs extend authored practice into concurrency, Redis, GenAI, and ML, and ten packs written on September 3 cover the data-at-scale and fast-reads units that open the second month (replication and sharding, consistent hashing, SSTables, Cassandra, DynamoDB, Memcached, Elasticsearch, Lucene, object storage, CloudFront): 33 packs, 116 groups, and 348 variants in total. Each authored session presents three to five exercises drawn from calculation, ordering, and scenario-decision tasks whose answer keys were checked against the lesson sources. The remaining 169 lesson sessions use packs derived from their existing source lessons, bringing the whole curriculum to 792 groups and 2,040 variants. `lib/skill-exercise-schema.ts` validates numeric answers, complete ordering permutations, matching pairs, and choice answers with option-specific feedback. `SkillPractice.tsx` renders numbers, keyboard-operable ordering, and role matching; scenario choices use the existing shared `InteractiveQuiz`. Concept context precedes every task, hints remain optional, and normal practice allows unlimited retries. Reviews rotate the chosen variant. Derived practice is explicitly marked with its source; it is not counted as newly hand-authored content.
