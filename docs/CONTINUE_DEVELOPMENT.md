...
For the next Claude session, start with [`CLAUDE_HANDOFF.md`](CLAUDE_HANDOFF.md). The verified production release is `278393767d9cc4e3b1c4d22a909b80cc4efa23af`, deployed to `https://www.systemdesigner.net/learn` as Vercel deployment `dpl_2C8agN6XvuwA5HS8NgNvw6jGe8B9` on September 6, 2026 (after `441aa5a4` and `1607a66b` on September 4). Live guest checks passed for the content API on newly authored GenAI and ML packs, the retired generated-practice path, and the second-month journey chapter through the live UI. Physical-device and live signed-in two-device checks remain outstanding.
