...
- Seventy-two lessons have 311 hand-authored groups and 933 variants: all 65 system-design lessons (so every guided-journey lesson day uses authored practice) plus concurrency, Redis, RAG, LLM serving, tokenization, multimodal budgets, ML framing, serving, and monitoring. Every other lesson uses matching and source decision tasks through the same schema; derived prompts name their lesson section and derived hints recall the key idea rather than the answer. Lesson sessions link their registry prerequisites.
