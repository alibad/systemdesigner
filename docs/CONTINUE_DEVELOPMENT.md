...
For the next Claude session, start with [`CLAUDE_HANDOFF.md`](CLAUDE_HANDOFF.md). The verified production release is `ab6e33a11b28c102b9e62a36eb428bc0c4c187f6`, deployed to `https://www.systemdesigner.net/learn` as Vercel deployment `dpl_C846vdccvefBjTG7AFnc7gbR2H4q` on September 6, 2026. It runs Next.js 15; `27839376` and `7324696c` earlier the same day added the GenAI and ML coding exercises and the multi-step runner. Live guest checks passed for the content API on newly authored GenAI and ML packs, the retired generated-practice path, and the second-month journey chapter through the live UI. Physical-device and live signed-in two-device checks remain outstanding.

The product direction is a substantial Duolingo-style engineering learning platform. `/learn` has four courses, 46 units, 273 sessions, and 236 skills. Every one of its 202 noncoding lesson sessions now uses hand-authored practice: 202 packs, 961 groups, 2,883 variants, across all four courses. No generated practice remains. The guided journey covers every session in all four courses across 326 study days in 15 parts. The catalog links 400 source-model references; later lessons open their models on demand, while the opening three use their compact models. The opening trail and its request/capacity/cache models remain the approved visual direction. Version 5 adds revision-aware quiz/exercise resume and recoverable recent coding drafts. Placement, adaptive review, the 326-study-day journey, and offline visited content remain available.
...
- A 326-study-day guided journey in 15 parts covers all 273 sessions in course order. Parts one to five walk system design and coding across 130 days, closing each of the first four months with a coding project. Parts six to ten add the 90 GenAI sessions over 103 days, and parts eleven to fourteen add the 72 ML sessions over 84 days. There are 53 review days (106 review tasks) and 25 milestones. Day IDs are stable, and the trail now ends when the whole curriculum is complete rather than handing off to a course page.
...
- Local-day goals (1–3 sessions), streaks, rotating review variants, and adaptive scheduling across all 235 skills. Full passes earn a delay; mistakes/hints return sooner. Partial answers do not increase recall strength.
...
Author the course outline, journey parts, and co-located sources, then run `pnpm generate:learning`. Do not hand-edit generated outputs (`catalog.json`, `sessions.json`, `journey.json`, checkpoints, or `daily-practice.generated.json`). `pnpm validate:learning` checks for drift and is included in `pnpm validate:content`, as is `pnpm validate:practice`, which enforces the authored-practice contract described in `scripts/validate-skill-exercises.mjs`. `pnpm qa:practice-variants` (`LEARNING_QA_VARIANT=1` or `2`) seeds prior completions so each lesson opens at its second or third review and answers those exercises through the real interface; the curriculum sweep only reaches the first variant. A new full lesson still requires the registry-first authoring workflow. Adding a course session around an existing lesson does not require a duplicate registry entry or body. To replace a derived pack, write `data/skill-exercises.json` beside the lesson, add it to `exerciseSources`, delete the stale generated file, and regenerate; variant IDs must be unique across all packs.
