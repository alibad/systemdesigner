...
- Forty-six lessons have 181 hand-authored groups and 543 variants: the first three design units, concurrency, Redis, RAG, LLM serving, tokenization, multimodal budgets, ML framing, serving, monitoring, the ten data-at-scale and fast-reads lessons that open month two, and the thirteen service-interfaces and asynchronous-systems lessons that follow (journey days 50–57 and 61–72). Every other lesson uses matching and source decision tasks through the same schema; derived prompts name their lesson section and derived hints recall the key idea rather than the answer. Lesson sessions link their registry prerequisites.
