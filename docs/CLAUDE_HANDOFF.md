...
- Released commit: `ab6e33a11b28c102b9e62a36eb428bc0c4c187f6` (`fix: upgrade to Next.js 15`), on top of `7324696c` (multi-step coding exercises) and `27839376` (`feat: add coding exercises for GenAI and machine learning`), on top of `1607a66b`/`441aa5a4` (the journey extension and exercise verification), `d8a1ee2c` (the authored-practice contract check), `dd26a9bd` (hand-authored practice for every lesson), and the earlier design-course and journey work. All are pushed to `origin/main`. Earlier production deployments remain rollback candidates in Vercel.
- Live app: `https://www.systemdesigner.net/learn`
- Vercel deployment: `dpl_C846vdccvefBjTG7AFnc7gbR2H4q` for `ab6e33a1` (immutable URL `https://systemdesigner-fjmz744vt-alibads-projects.vercel.app`), preceded by `dpl_2C8agN6XvuwA5HS8NgNvw6jGe8B9` for `27839376`, preceded by `dpl_2UCtPm3V7ujxwTbRmK72dKfFuRrQ` for `441aa5a4` and `dpl_CXX4HZeCzDqSVruWrnQWT4UdkEo1` for `dd26a9bd` on September 4.
...
- Coding exercises run in a worker inside an opaque-origin iframe. Most call one function; a multi-step exercise declares `exports` and drives an ordered `calls` sequence across several functions the learner writes, rebuilt per test.
...
- **Released.** Deployment `dpl_2C8agN6XvuwA5HS8NgNvw6jGe8B9` reached READY about four minutes after the push. Live guest checks: `/learn` and `/roadmap` returned 200, the trail reported 326 study days, and the new starter files and challenge JSON served from the content API. `pnpm qa:learning-journey` passed against the live domain. Logs: `.artifacts/coding-units/`.
...
- Unit tests: 377 passed, including a walk of all 326 study days through real prerequisite gates and coverage/ordering checks for every part.
