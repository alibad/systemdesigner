...
- Released commit: `dd26a9bd6a359fb005e6495aaeea5dd222ccf8da` (`feat: hand-author practice for every lesson and remove the answer-length tell`), on top of `00cbd003`/`cc175c2d` (the design-course packs), `3089d150` (service-interface and async packs, bounded review set) and `340dbbf0`/`041b246b`/`9cd7feab` (the 130-day journey). All are pushed to `origin/main`. Earlier production deployments remain rollback candidates in Vercel.
- Live app: `https://www.systemdesigner.net/learn`
- Vercel deployment: `dpl_CXX4HZeCzDqSVruWrnQWT4UdkEo1` for `dd26a9bd` (immutable URL `https://systemdesigner-m430pn3gu-alibads-projects.vercel.app`), preceded the same day by `dpl_U7qFKfFv6vy2GNGxYViW88b5qaL2` for `00cbd003`, `dpl_3QBrsQY54pRHFsoMTo7ids1ZE1m3` for `cc175c2d`, `dpl_q99sRX7hz1zT3cT4NCEw6k8f6zmU` for `3089d150`, and `dpl_DRT6oQduQth6ppk5LLQya9Jj9x9n` for `041b246b`.
...
2. **Calibrate the curriculum, do not expand it.** All 202 lessons are authored, so coverage work is done. What is unproven is difficulty: variant two and variant three of each group have not been walked in a browser, and no exercise has been checked against learner outcomes. Useful next steps are sampling variants for difficulty drift, checking that a group's three variants really are the same skill, and reviewing calculation answers a second time. If new lessons are added, follow the shared contract: five groups of one skill, three substantively different variants, at least one ordered process, two named mistakes per calculation, options whose length carries no signal, IDs namespaced by lesson, registered in `exerciseSources`. Do not inflate counts or relabel generated content “authored.”
